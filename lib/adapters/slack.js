...
module.exports = class SlackAdapter {
  ...
  text(text, channel) {
    this._queue.push((cb) => this.webClient.chat.postMessage(channel, text, { as_user: true }, (err) => cb(err)));
  }
  ...
  addReaction(react) {
    return new Promise((resolve, reject) => {
      ...
      this.webClient.reactions.add(react.emoji, options, (err) => {
        if (err) {
          return reject();
        }
  ...
  removeReaction(react) {
    return new Promise((resolve, reject) => {
      ...
      this.webClient.reactions.remove(react.emoji, options, (err) => err ? reject(err) : resolve());
    });
  }
